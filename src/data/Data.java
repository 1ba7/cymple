...
public class Data implements Graphable, Chartable {
	...
	public void update() {
		try {
			...
			if (chartData == null || chartData.getResolution() != resolution ||
				...
				for (int j = 0; index == userIndex ? j < 1 : j < users.length; j++) {
			...
			if (graphData == null || graphData.getResolution() != resolution ||
				...
				for (int i = 0; i < sampleRate; i++) {
					start = (short)(((position * (1 - resolution) + (double)i / sampleRate) - 0.5) * 0xFFFF);
					finish = (short)(((position * (1 - resolution) + (double)(i + 1) / sampleRate) - 0.5) * 0xFFFF);
